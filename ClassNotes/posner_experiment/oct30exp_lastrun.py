...
expName = 'oct30exp'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\psych\\Desktop\\posner experiment\\oct30exp_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('target_resp') is None:
        # initialise target_resp
        target_resp = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='target_resp',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    inst_text = visual.TextStim(win=win, name='inst_text',
        text='Press the button associated with the side the target is on. ',
        font='Arial',
    ...
    cue = visual.ImageStim(
        win=win,
        name='cue', 
        image='images/rocket.png', mask=None, anchor='center',
        ori=1.0, pos=(0, 0), draggable=False, size=(0.1, 0.1),
    ...
    target_resp = keyboard.Keyboard(deviceName='target_resp')
    ...
    instructions = data.Routine(
        name='instructions',
        components=[inst_text, start_resp],
    )
    ...
    while continueRoutine:
        ...
        if start_resp.status == STARTED and not waitOnFlip:
            theseKeys = start_resp.getKeys(keyList=['y','n','left','right','space'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisBlock in blocks:
        ...
        for thisTrial in trials:
            ...
            trial = data.Routine(
                name='trial',
                components=[cue, target, target_resp],
            )
            trial.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from code_2
            target_stim = ['images/green_alien.png', 'images/red_alien.png'] #list of possible target colours
            
            #shuffle the list 
            shuffle(target_stim)
            
            #pick a target stim
            tImage = target_stim[0]
            
            thisExp.addData('Target image', tImage)
            
            
            cue.setOri(cue_ori)
            target.setPos((target_x, 0))
            target.setOri(0.0)
            target.setImage(tImage)
            # create starting attributes for target_resp
            target_resp.keys = []
            target_resp.rt = []
            ...
            while continueRoutine:
                ...
                # if target_resp is starting this frame...
                if target_resp.status == NOT_STARTED and tThisFlip >= 0.6-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if target_resp.status == STARTED and not waitOnFlip:
                    theseKeys = target_resp.getKeys(keyList=['y','n','left','right','space'], ignoreKeys=["escape"], waitRelease=False)
                    ...
                    if len(_target_resp_allKeys):
                        ...
                        target_resp.duration = _target_resp_allKeys[-1].duration
                        # was this correct?
                        if (target_resp.keys == str(answer)) or (target_resp.keys == answer):
                            target_resp.corr = 1
                        else:
                            target_resp.corr = 0
                        # a response ends the routine
            ...
            if target_resp.keys != None:  # we had a response
                ...
                trials.addData('target_resp.duration', target_resp.duration)
            ...
            if target_resp.corr == 1: 
                fb_image_stim = 'images/star.png'
            else:
                fb_image_stim = 'images/asteroid.png'
            fbimage.setImage(fb_image_stim)
            ...
            while continueRoutine and routineTimer.getTime() < 0.5:
                ...
                # if fbimage is starting this frame...
                if fbimage.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'fbimage.started')
                    # update status
                    fbimage.status = STARTED
                    fbimage.setAutoDraw(True)
                
                # if fbimage is active this frame...
                if fbimage.status == STARTED:
                    # update params
                    pass
                
                # if fbimage is stopping this frame...
                if fbimage.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > fbimage.tStartRefresh + 0.5-frameTolerance:
                        # keep track of stop time/frame for later
                        fbimage.tStop = t  # not accounting for scr refresh
                        fbimage.tStopRefresh = tThisFlipGlobal  # on global time
                        fbimage.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'fbimage.stopped')
                        # update status
                        fbimage.status = FINISHED
                        fbimage.setAutoDraw(False)
